...
def get_set_of_single_category(category, url):
    ...
    for i in range(1,20):
        params['page'] = i
        print("카테고리 : {}, page : {}".format(category, params['page']))
...
def get_journalist(journalist_field):
    ...
    if writer_email is None:
        cursor.execute("SELECT * FROM tb_journalist WHERE press_id = 1 and name like %s LIMIT 1", writer_name)
        result = cursor.fetchone()
        if result is None:
            cursor.execute("INSERT INTO tb_journalist (press_id, name) VALUES (1, %s)", (writer_name))
            db.commit()
            cursor.execute("SELECT * FROM tb_journalist WHERE press_id = 1 and name like %s", writer_name)
            result = cursor.fetchone()
    else:
        ...
        result = cursor.fetchone()
        if result is None:
            cursor.execute("INSERT INTO tb_journalist (press_id, name, email) VALUES (1, %s, %s)", (writer_name, writer_email))
            db.commit()
            cursor.execute("SELECT * FROM tb_journalist WHERE email like %s", writer_email)
            result = cursor.fetchone()
